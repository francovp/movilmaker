...
public class Database {
	...
	public boolean ingresarEmpresaBD(Compania empresa) throws SQLException {
		if(c !=null )
		{
			...
			String sql = "INSERT INTO compania (nombre,id_compania) "
	...
	public boolean ingresarClienteBD(Cliente datosCliente) throws SQLException {
		if(c !=null )
		{
			...
			String sql = "INSERT INTO persona (nombre1,apellido1,id_compania,"
					+ "nombre2,fono_celular,fono_fijo,email,direccion1,direccion2,apellido2,rut,tipo,password) "
					...
					+  "','"+datosCliente.getRut()+"',"+datosCliente.getTipo()+",null);";
			stmt.executeUpdate(sql);
			stmt.close();
			// Se cierra conexión a la BD
			c.close();
	...
	public boolean ingresarAdminBD(Administrador datosAdmin) throws SQLException {
		if(c !=null )
		{
			...
			String sql = "INSERT INTO persona (nombre1,apellido1,id_compania,"
					+ "nombre2,fono_celular,fono_fijo,email,direccion1,direccion2,apellido2,rut,tipo,password) "
					+ "VALUES ('"+datosAdmin.getNombre1()+"','"+datosAdmin.getApellido1()
					+ "','"+datosAdmin.getIdCompania()+"','"+datosAdmin.getNombre2()
					+ "',"+datosAdmin.getFonoCel()+","+datosAdmin.getFonoFijo()+",'"+datosAdmin.getEmail()
					+ "',null,null,'"+datosAdmin.getApellido2()
					+  "','"+datosAdmin.getRut()+"',"+datosAdmin.getTipo()+",'"+datosAdmin.getPassword()+");";
			stmt.executeUpdate(sql);
			stmt.close();
			// Se cierra conexión a la BD
			c.close();
	...
	public Compania leerPersonasBD(Compania empresa) throws SQLException {
		if (c != null){
			...
			if(rs == null){
				...
				while (rs.next()) {
					// Se obtienen datos de personas y se verifica qué tipo de persona es la fila actual
					int tipo = rs.getInt("tipo");
					// y se asigna dependiendo del tipo
					if(tipo == 0){
						// Administrador
						Administrador admin = new Administrador(rs.getString("rut"),rs.getString("id_compania"),rs.getString("nombre1"),rs.getString("nombre2")
								,rs.getString("apellido1"),rs.getString("apellido2"),rs.getInt("fono_celular"),rs.getInt("fono_fijo"),rs.getString("email")
								,rs.getInt("tipo"),rs.getString("direccion1"),rs.getString("direccion2"), rs.getInt("deuda"), rs.getString("password"));
