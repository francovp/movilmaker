...
public class FrameAgregarEjecutivo extends JFrame {
	...
	public static void main(String[] args, Compania datosEmpresa) {
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				try {
					FrameAgregarEjecutivo frame = new FrameAgregarEjecutivo(datosEmpresa);
	 ...
	 * Create the frame.
	 */
	public FrameAgregarEjecutivo(Compania datosEmpresa) {
		setResizable(false);
		...
		// Boton que caputura todos los datos del Ejecutivo, crea objeto y
		// agrega a lista de Compania
		JButton btnAgregar = new JButton("Agregar");
		btnAgregar.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				Ejecutivo nuevoEjecutivo = null;

				// Comprobaciones de Datos ingresados
				if(comprobarIngreso(lblAviso)){					
					// Llama metodo para crear Ejecutivo
					nuevoEjecutivo = datosNuevaPersona(datosEmpresa);
					if (nuevoEjecutivo != null) {
						// Si el Ejecutivo se crea exitosamente se escribira
						// Ejecutivo en la BD
						try {
							// Creacion de conexion a base de datos
							Database.ingresarEjecutivoBD(nuevoEjecutivo);
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							System.err.println("Ejecutivo no se pudo escribir en la Base de Datos.\n"
									+ "\nDetalles de la excepci�n:");
							System.err.println(e1.getClass().getName() + ": " + e1.getMessage());
						}

						// // Para guardar Ejecutivo en un XML
						// // Objeto XML
						// XML xml = new XML();
						// if(xml.ingresarEjecutivoXML(datosEmpresa,
						// nuevoEjecutivo))
						// System.out.println("Ejecutivo guardado en XML.");
						// else System.err.println("Ejecutivo no fue
						// guardado en XML.");
						//
						// Muestra mensaje que el Ejecutivo fue ingresado
						// exitosamente!
						JOptionPane.showMessageDialog(null, "Ejecutivo creado con exito!", "Aviso",
								JOptionPane.INFORMATION_MESSAGE);
						dispose();
					} else {
						// Sino, se informa que el Ejecutivo ya existe y se
						// vuelve al menu
						lblAviso.setForeground(Color.RED);
						lblAviso.setText("Ejecutivo ya existe!");
					}
				}
			}
		});
	 ...
	 * atribuye a un objeto Ejecutivo que despues se asocia a la empresaç
	 * @param datosEmpresa - una referencia a la Compania
	 * @return un objeto Ejecutivo del administrador creado
	 **/
	public Ejecutivo datosNuevaPersona(Compania datosEmpresa) {
		String nombre1 = null, nombre2 = null, apellido1 = null, apellido2 = null, rut = null, email = null;
		int fono1 = 0, fono2 = 0;
		//Tipo = 2 indica que es Ejecutivo
		int tipo = 2; 
		...
		// Se crea Ejecutivo nuevo
		Ejecutivo adminNuevo = new Ejecutivo(rut, datosEmpresa.getRut(), nombre1, nombre2, apellido1, apellido2,
				fono1, fono2, email, tipo, null, null, 0, null);
		// Se ingresa Ejecutivo nuevo y se espera un resultado del ingreso

		if (datosEmpresa.getEjecutivos().validarAgregar(adminNuevo) == true){
